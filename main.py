...
def liste_activites(
    ...
    exclude_lieu: Optional[str] = Query(None),
    exclude_meteo: Optional[str] = Query(None),
    exclude_mois: Optional[str] = Query(None),
    ...
    if exclude_lieu:
        conditions.append("(a.lieu IS NULL OR a.lieu != ?)")
        params.append(exclude_lieu)

    if exclude_meteo:
        col = f"meteo_{exclude_meteo}"
    ...
    if exclude_attendu_ids:
        ...
        if ids:
            placeholders = ','.join('?' * len(ids))
            conditions.append(f"a.id NOT IN (SELECT activite_id FROM activite_attendu WHERE attendu_id IN ({placeholders}))")
            params += ids

    if exclude_tag_ids:
        ids = [int(x) for x in exclude_tag_ids.split(',') if x.strip().isdigit()]
        if ids:
            placeholders = ','.join('?' * len(ids))
            conditions.append(f"a.id NOT IN (SELECT activite_id FROM activite_tag WHERE tag_id IN ({placeholders}))")
            params += ids

    if exclude_cycle_ids:
        ids = [int(x) for x in exclude_cycle_ids.split(',') if x.strip().isdigit()]
        if ids:
            placeholders = ','.join('?' * len(ids))
            conditions.append(f"a.id NOT IN (SELECT activite_id FROM activite_cycle WHERE cycle_id IN ({placeholders}))")
            params += ids
    ...
    for cr in cycle_rows:
        cnt = conn.execute(
            f"""SELECT COUNT(DISTINCT a.id) FROM activite a
                INNER JOIN activite_cycle ac ON a.id = ac.activite_id
                {where_clause} AND ac.cycle_id = ?""",
            params + [cr["id"]]
        ).fetchone()[0]
        cycle_counts[cr["code"]] = cnt

    # Récupérer les résultats
    rows = conn.execute(
        f"""
        SELECT a.id, a.nom, a.lieu, a.duree_min, a.format_groupe,
               a.meteo_nuage, a.meteo_soleil, a.meteo_pluie, a.meteo_vent, a.meteo_nuit,
